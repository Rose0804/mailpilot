...
JSON.stringify(Mail.accounts().map(account => ({
  accountId: String(read(() => account.name(), "")),
...
const Mail = Application("Mail");
function read(call, fallback) { try { return call(); } catch (_) { return fallback; } }
const account = Mail.accounts().find(item => String(read(() => item.name(), "")) === ${JSON.stringify(accountId)});
JSON.stringify(account ? account.mailboxes().map(mailbox => ({
  accountId: ${JSON.stringify(accountId)},
  id: String(read(() => mailbox.name(), "")),
...
function read(call, fallback) { try { return call(); } catch (_) { return fallback; } }
const account = Mail.accounts().find(item => String(read(() => item.name(), "")) === ${JSON.stringify(accountId)});
const mailbox = account && account.mailboxes().find(item => String(read(() => item.name(), "")) === ${JSON.stringify(mailboxId)});
